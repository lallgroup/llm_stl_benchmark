...
NL_CRITIQUE_PROMPT = (
    ...
    "(no explanations). If there are no errors, output \"No changes needed.\"."
)

NL_CRITIQUE_NL_PLAN_PROMPT =  (
    "Check your plan to make sure it meets all the requirements and contains "
    "no errors. Revise your plan. Output only the revised plan as a step-by-step list of actions, with each step separated by \\n\\n."
    "(no explanations). If there are no errors, output \"No changes needed.\"."
)
...
def build_nl_feedback(results: list[PropertyResult], nl_plan_prompt: bool = False) -> Optional[str]:
    ...
    If nl_plan_prompt is True, the prompt is formatted as a step-by-step list of actions and is not code.
    """
    ...
    if nl_plan_prompt:
        return NL_CRITIQUE_NL_PLAN_PROMPT
    else:
...
def build_nl_feedback_verifier_aware(results: list[PropertyResult]) -> Optional[str]:
    ...
    This is only valid for coding plans because the verifier can only run on code."""
...
def run_nl_critique_loop(
    ...
    *,
    ...
    """
    assert not (verifier_aware and nl_plan_prompt), "Verifier-aware mode is only valid for coding plans because the verifier can only run on code."

    builder = None
    if verifier_aware:
        builder = build_nl_feedback_verifier_aware
    elif nl_plan_prompt:
        builder = partial(build_nl_feedback, nl_plan_prompt=True)
    else:
        builder = build_nl_feedback
    ...
    return run_verified_loop(
        ...
        condition="nl-critique" + ("+vaware" if verifier_aware else "") + ("-nl-plan" if nl_plan_prompt else ""),
    )
